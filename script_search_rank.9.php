	...
	while($row=mysql_fetch_array($table_result)){
		...
		for($index=0;$index<$num_fields;$index++){
			...
			if(strlen($row[$index])>4){
				...
				mysql_query($sql);
				$result_id_row = mysql_fetch_array(mysql_query("SELECT `result_id` FROM `search_rank` WHERE `table_id`='$escaped_table_id' AND `record_id`='$escaped_record_id' AND `field_id`='$index'"));
